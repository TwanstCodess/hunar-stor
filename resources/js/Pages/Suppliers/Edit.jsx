...
import { ArrowRight, Save, Building, Phone, Mail, MapPin, FileText, Briefcase, DollarSign, AlertCircle, Clock } from 'lucide-react';
...
export default function Edit({ supplier }) {
  ...
  const handleSubmit = (e) => {
    ...
    router.put(`/suppliers/${supplier.id}`, data, {
      onSuccess: () => {
        setProcessing(false);
      },
      onError: (errors) => {
  ...
  return (
    <AuthenticatedLayout>
      <PageHeader
        title="دەستکاریکردنی دابینکەر"
        subtitle={`دەستکاریکردنی: ${supplier.name}`}
      />

      <div className="max-w-4xl">
        {/* کارتەکانی زانیاری دابینکەر */}
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-3">
          <Card className="border border-blue-200 bg-blue-50">
            <div className="flex items-center gap-4 p-4">
              ...
              <div>
                <div className="mb-1 text-sm text-blue-700">کاتی دروستکردن</div>
                <div className="font-medium text-blue-900">{formatDate(supplier.created_at)}</div>
              </div>
            </div>
          </Card>
        ...
        {/* کارتی ئامۆژگاری */}
        <div className="mt-6">
          <Card className={`border ${supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'border-red-200 bg-red-50' : 'border-yellow-100 bg-yellow-50'}`}>
            <div className="flex items-start gap-3">
              <div className={`flex items-center justify-center w-10 h-10 rounded-lg ${supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'bg-red-100' : 'bg-yellow-100'}`}>
                <AlertCircle className={`w-5 h-5 ${supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'text-red-600' : 'text-yellow-600'}`} />
              </div>
              <div className="flex-1">
                <h4 className={`font-medium ${supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'text-red-900' : 'text-yellow-900'}`}>
                  {supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'ئاگاداری' : 'تێبینی گرنگ'}
                </h4>
                <p className={`mt-1 text-sm ${supplier.balance_iqd > 0 || supplier.balance_usd > 0 ? 'text-red-700' : 'text-yellow-700'}`}>
                  {supplier.balance_iqd > 0 || supplier.balance_usd > 0
                    ? 'ئەم دابینکەرە قەرزی لەسەرە. دەستکاریکردنی زانیاری کاریگەری لەسەر قەرزەکانی نابێت. بۆ دەستکاریکردنی قەرزەکان، پەڕەی کڕین یان پارەدان بەکاربهێنە.'
                    : 'دەستکاریکردنی زانیاری دابینکەر کار دەکاتە سەر هەموو کڕین و پارەدانی داواکراوەکانی داھاتوو.'
                  }
                </p>
              </div>
            </div>
