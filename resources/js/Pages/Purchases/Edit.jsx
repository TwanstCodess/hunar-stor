...
import {
  ...
  Search, Package, AlertTriangle, ShoppingCart, Info, Image as ImageIcon
} from 'lucide-react';
...
export default function Edit({ purchase, suppliers, products }) {
  const [data, setData] = useState({
    ...
    items: purchase.items.map(item => ({
      ...
      unit_label: item.product.unit_label || 'دانە'
    }))
  });
  ...
  const handleSubmit = (e) => {
    ...
    if (data.purchase_type === 'credit' && !data.supplier_id) {
      if (!confirm('کڕینەکەت وەک قەرزە بێ دابینکەر! دڵنیایت لە نوێکردنەوە؟')) {
        return;
      }
    }
    ...
    router.put(`/purchases/${purchase.id}`, submitData, {
      onError: (errors) => {
        setErrors(errors);
      }
    });
  ...
  return (
    <AuthenticatedLayout>
      <PageHeader
        title={`دەستکاری کڕین ${purchase.invoice_number}`}
        subtitle="نوێکردنەوەی زانیاریەکانی کڕین"
      />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setData({ ...data, purchase_type: e.target.value })}
                      className="w-full border-gray-300 rounded-lg focus:border-blue-500 focus:ring-blue-500"
                    >
